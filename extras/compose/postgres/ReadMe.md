...
Original-Verzeichnis `init\parkhaus\sql` bzw. `init\parkhaus\tls` in das Named Volume
...
das Named Volume `pg_init` kopiert. Danach wird das Verzeichnis `/tablespace/parkhaus`
...
DB `parkhaus` mit dem DB-User `parkhaus`anzulegen. Mit dem 2. Skript wird das
Schema `parkhaus` mit dem DB-User `parkhaus` als _Owner_ angelegt:

```shell
    docker compose exec db bash
        psql --dbname=postgres --username=postgres --file=/init/parkhaus/sql/create-db.sql
        psql --dbname=parkhaus --username=parkhaus --file=/init/parkhaus/sql/create-schema.sql
        exit
    docker compose down
```
...
Mit der Erweiterung _PostgreSQL_ für VS Code kann man die Datenbank `parkhaus` und
...
Im Untermenü _Databases_ von der Verbindung sieht man dann z.B. die Datenbank `parkhaus`
mit dem gleichnamigen Schema `parkhaus` und die Datenbank `postgres`.
Ebenso kann man man unter _Roles_ den DB-User `parkhaus` und den Superuser `postgres`
...
- Wie kann ich parkhausen aus dem Postleitzahlengebiet "1" selektieren?
- Den Benutzernamen benötige ich nicht.
- Wie kann ich die Daten der 1. Abfrage als CSV exportieren?

**BEACHTE**:

- _parkhausen_ als Plural der DB-Tabelle `parkhaus`.
